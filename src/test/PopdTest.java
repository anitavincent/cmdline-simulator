...
public class PopdTest {
  ...
  public void testNoDir() {
    popd.execute(arguments, eh, validator, resource);
    result = out.consumeStdError();
    assertEquals("The directory stack is empty", result);
  }

  @Test
  public void testThreeDirs() {
    ds.push(directoryA);
    ds.push(directoryB);
    ds.push(directoryC);
    popd.execute(arguments, eh, validator, resource);
    assertEquals(dc.getWorkingDirectory(), directoryC);
  }
  ...
  public void testOneDir() {
    ds.push(directoryA);
    popd.execute(arguments, eh, validator, resource);
    assertEquals(dc.getWorkingDirectory(), directoryA);
  }
